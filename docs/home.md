...
Use the navigation below to explore topics. Links use the GitLab wiki page style, so you can jump between pages like [Getting Started](Getting-Started) or [Server-Side Reporting](Server-Side-Reporting).
...
If you only need tracking, install this package alone. If you need dashboards, install the base too. See [Installation](Installation) for the full matrix and [Graceful Degradation](Server-Side-Reporting-Graceful-Degradation) for what happens when the base is missing.

## What this package does not do

- It does **not** implement the OAuth flow. That lives in the base [`artisanpack-ui/google`](https://github.com/ArtisanPack-UI/google) package — see its [Getting Started](Getting-Started) guide.
